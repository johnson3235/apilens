  ...
  return <main className="app qa-shell">
    ...
    <div className="workspace">
      ...
      <section id="main-content" className="main-stage" tabIndex={-1}>
        ...
        {view === 'traces' ? <div className="scroll view-page"><ViewHeader overline="CORRELATION" title="Distributed traces" description="Browser, SDK and proxy spans grouped into customer journeys." />{data.traces.length ? <div className="trace-cards">{data.traces.map((trace, index) => <article className="trace-card" key={trace.traceId}><span className="badge accent">Trace {index + 1}</span><strong className="mono">{trace.traceId}</strong><p>{trace.spanCount} correlated span(s)</p></article>)}</div> : <EmptyState title="No traces captured" copy="Enable trace headers for a controlled environment, start a session, and complete a journey." action="Open settings" onAction={() => setView('settings')} />}</div> : null}
...
const customer = await response.json();`,
  next: `// lib/apilens.ts — server runtime only
import { ApiLensSDK } from '@apilens/node-sdk';

const apilens = new ApiLensSDK({
  serviceName: 'vois-ie-web-bff',
  reporterUrl: process.env.APILENS_REPORTER_URL,
  enabled: process.env.APILENS_ENABLED === 'true',
});

export default apilens;

// .env.local (controlled QA environment)
// APILENS_ENABLED=true
// APILENS_REPORTER_URL=http://127.0.0.1:7317`,
...
function IntegrationSetup({ state, onView, onNotice }: { state: ReturnType<typeof usePanelData>['state']; onView: (view: View) => void; onNotice: (notice: string) => void }): JSX.Element {
  ...
  const details = {
    ...
    next: { label: 'Next.js SSR / BFF', eyebrow: 'OPTIONAL SERVER SDK', title: 'Connect calls made on the server', copy: 'Use the Node SDK only for route handlers, server actions, SSR or BFF calls that the browser extension cannot observe.', steps: ['Install @apilens/node-sdk in the server application.', 'Create one server-only SDK singleton using the configuration below.', 'Enable it only in a controlled QA environment.', 'Start the local agent, then verify a journey in Traces.'] },
...
function FeatureAcademy({ onView }: { onView: (view: View) => void }): JSX.Element {
  const lessons: Array<{ title: string; value: string; steps: string[]; result: string; target: View; safety?: string }> = [
    ...
    { title: 'Distributed Traces', value: 'Connect browser calls to controlled backend services.', steps: ['Enable trace headers only where CORS permits.', 'Enable and connect the local QA agent.', 'Install the Node SDK middleware before application routes.'], result: 'One browser/server waterfall with service boundaries.', target: 'traces', safety: 'Keep server integration off for ordinary sites.' },
